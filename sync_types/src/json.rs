...
use crate::{
    AuthenticationToken, ClientMessage, IdentityVersion, LogLinesMessage, Query, QueryId,
    QuerySetModification, SerializedQueryJournal, ServerMessage, SessionRequestSeqNumber,
    StateModification, StateVersion, Timestamp, UserIdentifier, UserIdentityAttributes,
    types::{ClientEvent, ErrorPayload},
};
...
fn string_to_u64(s: &str) -> anyhow::Result<u64> {
    let bytes: [u8; 8] = base64::decode(s)
        .or(Err(anyhow::anyhow!("some kind of decode error")))?
        .try_into()
...
impl From<StateModification> for JsonValue {
    fn from(m: StateModification) -> Self {
        match m {
        ...
        let result = match s {
            ServerMessageJson::Transition {
                ...
                modifications: modifications
                    ...
                    .collect::<anyhow::Result<Vec<StateModification>>>()?,
            },
            ServerMessageJson::MutationResponse {
                ...
                let result = if success {
                    ...
                    Err(if let Some(data) = error_data {
                        ErrorPayload::ErrorData { message: msg, data }
                    } else {
