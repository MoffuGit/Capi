    ...
    view! {
        <ToolPortalPrimitive>
            <ToolContentPrimitive
                ...
                class=Signal::derive(
                    move || tw_merge!(
                            "bg-primary border-primary border-0 data-[state=undefined]:opacity-0 text-primary-foreground ease-out-quad duration-200 animate-in fade-in-0 zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 z-50 w-fit origin-(--radix-tooltip-content-transform-origin) rounded-md px-3 py-1.5 text-xs text-balance",
                            class.get()
                    ))
            >
                {children.get_value()()}
